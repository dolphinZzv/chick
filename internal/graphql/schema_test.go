...
func setupTestResolver(t *testing.T) *Resolver {
	...
	return NewResolver(projectSvc, agentSvc, issueSvc, commentSvc, proposalSvc, taskSvc, workflowSvc, feedbackSvc, nil, nil, bus, false)
}
...
func TestGraphQL_IssueIntegration(t *testing.T) {
	...
	issue, err := r.Mutation().CreateIssue(ctx, project.ID, "Test Issue", strPtr("body"), PriorityMedium, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
...
func TestGraphQL_TransitionIssue(t *testing.T) {
	...
	issue, err := r.Mutation().CreateIssue(ctx, project.ID, "Transitions", nil, PriorityLow, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("create issue: %v", err)
	}
...
func TestGraphQL_AddComment(t *testing.T) {
	...
	issue, err := r.Mutation().CreateIssue(ctx, project.ID, "C", nil, PriorityLow, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("create issue: %v", err)
	}
...
func TestGraphQL_Timeline(t *testing.T) {
	...
	issue, err := r.Mutation().CreateIssue(ctx, project.ID, "T", nil, PriorityLow, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("create issue: %v", err)
	}
...
func TestGraphQL_CommentsQuery(t *testing.T) {
	...
	issue, err := r.Mutation().CreateIssue(ctx, project.ID, "C2", nil, PriorityLow, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("create issue: %v", err)
	}
	...
	comments, err := r.Query().Comments(ctx, &issue.ID, nil, nil, nil)
	if err != nil {
...
func TestGraphQL_NewHandlerCreatesHTTPHandler(t *testing.T) {
	r := setupTestResolver(t)
	h := NewHandler(r.ProjectSvc, r.AgentSvc, r.IssueSvc, r.CommentSvc, r.ProposalSvc, r.TaskSvc, r.WorkflowSvc, r.FeedbackSvc, r.WebhookSvc, r.NotifSvc, r.EventBus, false)
	if h == nil {
