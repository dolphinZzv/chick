...
export function WebhookSettings() {
  ...
  const [members, setMembers] = useState<Array<{ agent: { id: string; name: string }; role: string }>>([]);
  ...
  const fetchMembers = useCallback(async () => {
    if (!id) return;
    try {
      const res = await gql(`query project($id: ID!) { project(id: $id) { members { agent { id name } role } } }`, { id });
      if (!res.errors) setMembers(res.data.project.members || []);
    } catch { /* ignore */ }
  }, [id]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!id || !newName.trim() || !newAgentID) return;
    setCreating(true);
    try {
      const res = await gql(
        `mutation createWebhook($pid: ID!, $aid: ID!, $name: String!) { createWebhook(projectID: $pid, agentID: $aid, name: $name) { webhook { secret } curlExample } }`,
        { pid: id, aid: newAgentID, name: newName }
      );
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => setCreateOpen(true)} disabled={members.length === 0}><Plus className="mr-1 h-4 w-4" />新建 Webhook</Button>
      </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">绑定 Agent</label>
              <Select value={newAgentID} onValueChange={setNewAgentID} required>
                <SelectTrigger><SelectValue placeholder="选择 Agent" /></SelectTrigger>
                <SelectContent>
                  {members.map(m => (
                    <SelectItem key={m.agent.id} value={m.agent.id}>{m.agent.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
